...
DumpSsaTdpdVTK::DumpSsaTdpdVTK(LAMMPS *lmp, int narg, char **arg) :
  DumpSsaTdpd(lmp, narg, arg)
{
...
void DumpSsaTdpdVTK::init_style()
{
  ...
  header_choice = &DumpSsaTdpdVTK::header_vtk;

  if (vtk_file_format == VTP || vtk_file_format == PVTP)
    write_choice = &DumpSsaTdpdVTK::write_vtp;
  else if (vtk_file_format == VTU || vtk_file_format == PVTU)
    write_choice = &DumpSsaTdpdVTK::write_vtu;
  else
    write_choice = &DumpSsaTdpdVTK::write_vtk;
...
void DumpSsaTdpdVTK::write_header(bigint)
{
}

/* ---------------------------------------------------------------------- */

void DumpSsaTdpdVTK::header_vtk(bigint)
{
}

/* ---------------------------------------------------------------------- */

int DumpSsaTdpdVTK::count()
{
  n_calls_ = 0;
...
void DumpSsaTdpdVTK::pack(tagint *ids)
{
  int n = 0;
...
void DumpSsaTdpdVTK::write_data(int n, double *mybuf)
{
...
void DumpSsaTdpdVTK::setFileCurrent() {
  delete [] filecurrent;
  filecurrent = NULL;
...
void DumpSsaTdpdVTK::buf2arrays(int n, double *mybuf)
{
...
void DumpSsaTdpdVTK::prepare_domain_data(vtkRectilinearGrid *rgrid)
{
...
void DumpSsaTdpdVTK::prepare_domain_data_triclinic(vtkUnstructuredGrid *hexahedronGrid)
{
...
void DumpSsaTdpdVTK::write_domain_vtk()
{
...
void DumpSsaTdpdVTK::write_domain_vtk_triclinic()
{
...
void DumpSsaTdpdVTK::write_domain_vtr()
{
  vtkSmartPointer<vtkRectilinearGrid> rgrid = vtkSmartPointer<vtkRectilinearGrid>::New();
  prepare_domain_data(rgrid.GetPointer());
...
void DumpSsaTdpdVTK::write_domain_vtu_triclinic()
{
  vtkSmartPointer<vtkUnstructuredGrid> hexahedronGrid = vtkSmartPointer<vtkUnstructuredGrid>::New();
  prepare_domain_data_triclinic(hexahedronGrid.GetPointer());
...
void DumpSsaTdpdVTK::write_vtk(int n, double *mybuf)
{
  ++n_calls_;
...
void DumpSsaTdpdVTK::write_vtp(int n, double *mybuf)
{
  ++n_calls_;
...
void DumpSsaTdpdVTK::write_vtu(int n, double *mybuf)
{
  ++n_calls_;
...
void DumpSsaTdpdVTK::reset_vtk_data_containers()
{
...
int DumpSsaTdpdVTK::parse_fields(int narg, char **arg)
{

  pack_choice[X] = &DumpSsaTdpdVTK::pack_x;
  vtype[X] = DOUBLE;
  name[X] = "x";
  pack_choice[Y] = &DumpSsaTdpdVTK::pack_y;
  vtype[Y] = DOUBLE;
  name[Y] = "y";
  pack_choice[Z] = &DumpSsaTdpdVTK::pack_z;
  vtype[Z] = DOUBLE;
  name[Z] = "z";
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      pack_choice[ID] = &DumpSsaTdpdVTK::pack_id;
      vtype[ID] = INT;
      name[ID] = arg[iarg];
    } else if (strcmp(arg[iarg],"mol") == 0) {
      if (!atom->molecule_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[MOL] = &DumpSsaTdpdVTK::pack_molecule;
      vtype[MOL] = INT;
      name[MOL] = arg[iarg];
    } else if (strcmp(arg[iarg],"proc") == 0) {
      pack_choice[PROC] = &DumpSsaTdpdVTK::pack_proc;
      vtype[PROC] = INT;
      name[PROC] = arg[iarg];
    } else if (strcmp(arg[iarg],"procp1") == 0) {
      pack_choice[PROCP1] = &DumpSsaTdpdVTK::pack_procp1;
      vtype[PROCP1] = INT;
      name[PROCP1] = arg[iarg];
    } else if (strcmp(arg[iarg],"type") == 0) {
      pack_choice[TYPE] = &DumpSsaTdpdVTK::pack_type;
      vtype[TYPE] = INT;
      name[TYPE] =arg[iarg];
    } else if (strcmp(arg[iarg],"element") == 0) {
      pack_choice[ELEMENT] = &DumpSsaTdpdVTK::pack_type;
      vtype[ELEMENT] = STRING;
      name[ELEMENT] = arg[iarg];
    } else if (strcmp(arg[iarg],"mass") == 0) {
      pack_choice[MASS] = &DumpSsaTdpdVTK::pack_mass;
      vtype[MASS] = DOUBLE;
      name[MASS] = arg[iarg];
      ...
      if (domain->triclinic) pack_choice[XS] = &DumpSsaTdpdVTK::pack_xs_triclinic;
      else pack_choice[XS] = &DumpSsaTdpdVTK::pack_xs;
      vtype[XS] = DOUBLE;
      name[XS] = arg[iarg];
    } else if (strcmp(arg[iarg],"ys") == 0) {
      if (domain->triclinic) pack_choice[YS] = &DumpSsaTdpdVTK::pack_ys_triclinic;
      else pack_choice[YS] = &DumpSsaTdpdVTK::pack_ys;
      vtype[YS] = DOUBLE;
      name[YS] = arg[iarg];
    } else if (strcmp(arg[iarg],"zs") == 0) {
      if (domain->triclinic) pack_choice[ZS] = &DumpSsaTdpdVTK::pack_zs_triclinic;
      else pack_choice[ZS] = &DumpSsaTdpdVTK::pack_zs;
      vtype[ZS] = DOUBLE;
      name[ZS] = arg[iarg];
    } else if (strcmp(arg[iarg],"xu") == 0) {
      if (domain->triclinic) pack_choice[XU] = &DumpSsaTdpdVTK::pack_xu_triclinic;
      else pack_choice[XU] = &DumpSsaTdpdVTK::pack_xu;
      vtype[XU] = DOUBLE;
      name[XU] = arg[iarg];
    } else if (strcmp(arg[iarg],"yu") == 0) {
      if (domain->triclinic) pack_choice[YU] = &DumpSsaTdpdVTK::pack_yu_triclinic;
      else pack_choice[YU] = &DumpSsaTdpdVTK::pack_yu;
      vtype[YU] = DOUBLE;
      name[YU] = arg[iarg];
    } else if (strcmp(arg[iarg],"zu") == 0) {
      if (domain->triclinic) pack_choice[ZU] = &DumpSsaTdpdVTK::pack_zu_triclinic;
      else pack_choice[ZU] = &DumpSsaTdpdVTK::pack_zu;
      vtype[ZU] = DOUBLE;
      name[ZU] = arg[iarg];
    } else if (strcmp(arg[iarg],"xsu") == 0) {
      if (domain->triclinic) pack_choice[XSU] = &DumpSsaTdpdVTK::pack_xsu_triclinic;
      else pack_choice[XSU] = &DumpSsaTdpdVTK::pack_xsu;
      vtype[XSU] = DOUBLE;
      name[XSU] = arg[iarg];
    } else if (strcmp(arg[iarg],"ysu") == 0) {
      if (domain->triclinic) pack_choice[YSU] = &DumpSsaTdpdVTK::pack_ysu_triclinic;
      else pack_choice[YSU] = &DumpSsaTdpdVTK::pack_ysu;
      vtype[YSU] = DOUBLE;
      name[YSU] = arg[iarg];
    } else if (strcmp(arg[iarg],"zsu") == 0) {
      if (domain->triclinic) pack_choice[ZSU] = &DumpSsaTdpdVTK::pack_zsu_triclinic;
      else pack_choice[ZSU] = &DumpSsaTdpdVTK::pack_zsu;
      vtype[ZSU] = DOUBLE;
      name[ZSU] = arg[iarg];
    } else if (strcmp(arg[iarg],"ix") == 0) {
      pack_choice[IX] = &DumpSsaTdpdVTK::pack_ix;
      vtype[IX] = INT;
      name[IX] = arg[iarg];
    } else if (strcmp(arg[iarg],"iy") == 0) {
      pack_choice[IY] = &DumpSsaTdpdVTK::pack_iy;
      vtype[IY] = INT;
      name[IY] = arg[iarg];
    } else if (strcmp(arg[iarg],"iz") == 0) {
      pack_choice[IZ] = &DumpSsaTdpdVTK::pack_iz;
      vtype[IZ] = INT;
      name[IZ] = arg[iarg];

    } else if (strcmp(arg[iarg],"vx") == 0) {
      pack_choice[VX] = &DumpSsaTdpdVTK::pack_vx;
      vtype[VX] = DOUBLE;
      name[VX] = arg[iarg];
    } else if (strcmp(arg[iarg],"vy") == 0) {
      pack_choice[VY] = &DumpSsaTdpdVTK::pack_vy;
      vtype[VY] = DOUBLE;
      name[VY] = arg[iarg];
    } else if (strcmp(arg[iarg],"vz") == 0) {
      pack_choice[VZ] = &DumpSsaTdpdVTK::pack_vz;
      vtype[VZ] = DOUBLE;
      name[VZ] = arg[iarg];


    } else if (strcmp(arg[iarg],"fx") == 0) {
      pack_choice[FX] = &DumpSsaTdpdVTK::pack_fx;
      vtype[FX] = DOUBLE;
      name[FX] = arg[iarg];
    } else if (strcmp(arg[iarg],"fy") == 0) {
      pack_choice[FY] = &DumpSsaTdpdVTK::pack_fy;
      vtype[FY] = DOUBLE;
      name[FY] = arg[iarg];
    } else if (strcmp(arg[iarg],"fz") == 0) {
      pack_choice[FZ] = &DumpSsaTdpdVTK::pack_fz;
      vtype[FZ] = DOUBLE;
      name[FZ] = arg[iarg];
    } else if (strcmp(arg[iarg],"q") == 0) {
      if (!atom->q_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[Q] = &DumpSsaTdpdVTK::pack_q;
      vtype[Q] = DOUBLE;
      name[Q] = arg[iarg];
    } else if (strcmp(arg[iarg],"mux") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[MUX] = &DumpSsaTdpdVTK::pack_mux;
      vtype[MUX] = DOUBLE;
      name[MUX] = arg[iarg];
    } else if (strcmp(arg[iarg],"muy") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[MUY] = &DumpSsaTdpdVTK::pack_muy;
      vtype[MUY] = DOUBLE;
      name[MUY] = arg[iarg];
    } else if (strcmp(arg[iarg],"muz") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[MUZ] = &DumpSsaTdpdVTK::pack_muz;
      vtype[MUZ] = DOUBLE;
      name[MUZ] = arg[iarg];
    } else if (strcmp(arg[iarg],"mu") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[MU] = &DumpSsaTdpdVTK::pack_mu;
      vtype[MU] = DOUBLE;
      name[MU] = arg[iarg];
      ...
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[RADIUS] = &DumpSsaTdpdVTK::pack_radius;
      vtype[RADIUS] = DOUBLE;
      name[RADIUS] = arg[iarg];
    } else if (strcmp(arg[iarg],"diameter") == 0) {
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[DIAMETER] = &DumpSsaTdpdVTK::pack_diameter;
      vtype[DIAMETER] = DOUBLE;
      name[DIAMETER] = arg[iarg];
    } else if (strcmp(arg[iarg],"omegax") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[OMEGAX] = &DumpSsaTdpdVTK::pack_omegax;
      vtype[OMEGAX] = DOUBLE;
      name[OMEGAX] = arg[iarg];
    } else if (strcmp(arg[iarg],"omegay") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[OMEGAY] = &DumpSsaTdpdVTK::pack_omegay;
      vtype[OMEGAY] = DOUBLE;
      name[OMEGAY] = arg[iarg];
    } else if (strcmp(arg[iarg],"omegaz") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[OMEGAZ] = &DumpSsaTdpdVTK::pack_omegaz;
      vtype[OMEGAZ] = DOUBLE;
      name[OMEGAZ] = arg[iarg];
    } else if (strcmp(arg[iarg],"angmomx") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[ANGMOMX] = &DumpSsaTdpdVTK::pack_angmomx;
      vtype[ANGMOMX] = DOUBLE;
      name[ANGMOMX] = arg[iarg];
    } else if (strcmp(arg[iarg],"angmomy") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[ANGMOMY] = &DumpSsaTdpdVTK::pack_angmomy;
      vtype[ANGMOMY] = DOUBLE;
      name[ANGMOMY] = arg[iarg];
    } else if (strcmp(arg[iarg],"angmomz") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[ANGMOMZ] = &DumpSsaTdpdVTK::pack_angmomz;
      vtype[ANGMOMZ] = DOUBLE;
      name[ANGMOMZ] = arg[iarg];
    } else if (strcmp(arg[iarg],"tqx") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[TQX] = &DumpSsaTdpdVTK::pack_tqx;
      vtype[TQX] = DOUBLE;
      name[TQX] = arg[iarg];
    } else if (strcmp(arg[iarg],"tqy") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[TQY] = &DumpSsaTdpdVTK::pack_tqy;
      vtype[TQY] = DOUBLE;
      name[TQY] = arg[iarg];
    } else if (strcmp(arg[iarg],"tqz") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[TQZ] = &DumpSsaTdpdVTK::pack_tqz;
      vtype[TQZ] = DOUBLE;
      name[TQZ] = arg[iarg];
      ...
      pack_choice[i] = &DumpSsaTdpdVTK::pack_C;
      vtype[i] = DOUBLE;
      ...
      if (ptr) {
        if (suffix[strlen(suffix)-1] != ']')
          error->all(FLERR,"Invalid attribute in dump ssa_tdpd/vtk command");
    ...
    // if no trailing [], then arg is set to 0, else arg is int between []
      ...
      pack_choice[ATTRIBUTES+i] = &DumpSsaTdpdVTK::pack_compute;
    ...
    // if no trailing [], then arg is set to 0, else arg is between []
      ...
      pack_choice[ATTRIBUTES+i] = &DumpSsaTdpdVTK::pack_fix;
      vtype[ATTRIBUTES+i] = DOUBLE;
    ...
    // variable value = v_name
      ...
      pack_choice[ATTRIBUTES+i] = &DumpSsaTdpdVTK::pack_variable;
      vtype[ATTRIBUTES+i] = DOUBLE;
    ...
    // custom per-atom floating point value = d_ID
      ...
      pack_choice[ATTRIBUTES+i] = &DumpSsaTdpdVTK::pack_custom;
      vtype[ATTRIBUTES+i] = DOUBLE;
...
void DumpSsaTdpdVTK::identify_vectors()
{
  // detect vectors
...
int DumpSsaTdpdVTK::add_compute(char *id)
{
  int icompute;
...
int DumpSsaTdpdVTK::add_fix(char *id)
{
  int ifix;
...
int DumpSsaTdpdVTK::add_variable(char *id)
{
  int ivariable;
...
int DumpSsaTdpdVTK::add_custom(char *id, int flag)
{
  int icustom;
...
int DumpSsaTdpdVTK::modify_param(int narg, char **arg)
{
...
bigint DumpSsaTdpdVTK::memory_usage()
{
...
void DumpSsaTdpdVTK::pack_compute(int n)
{
...
void DumpSsaTdpdVTK::pack_fix(int n)
{
...
void DumpSsaTdpdVTK::pack_variable(int n)
{
...
void DumpSsaTdpdVTK::pack_custom(int n)
{
...
/*----------------------------------------------------------------------- */
//added
void DumpSsaTdpdVTK::pack_C(int n)
{
  double **C = atom->C;
  int index = argindex[current_pack_choice_key];
