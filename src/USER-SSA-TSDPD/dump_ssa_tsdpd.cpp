...
DumpSsaTsdpd::DumpSsaTsdpd(LAMMPS *lmp, int narg, char **arg) :
  Dump(lmp, narg, arg),
...
void DumpSsaTsdpd::init_style()
{
  ...
  if (binary && domain->triclinic == 0)
    header_choice = &DumpSsaTsdpd::header_binary;
  else if (binary && domain->triclinic == 1)
    header_choice = &DumpSsaTsdpd::header_binary_triclinic;
  else if (!binary && domain->triclinic == 0)
    header_choice = &DumpSsaTsdpd::header_item;
  else if (!binary && domain->triclinic == 1)
    header_choice = &DumpSsaTsdpd::header_item_triclinic;

  if (binary) write_choice = &DumpSsaTsdpd::write_binary;
  else if (buffer_flag == 1) write_choice = &DumpSsaTsdpd::write_string;
  else write_choice = &DumpSsaTsdpd::write_lines;
...
void DumpSsaTsdpd::write_header(bigint ndump)
{
...
void DumpSsaTsdpd::header_binary(bigint ndump)
{
...
void DumpSsaTsdpd::header_binary_triclinic(bigint ndump)
{
  fwrite(&update->ntimestep,sizeof(bigint),1,fp);
  fwrite(&ndump,sizeof(bigint),1,fp);
  fwrite(&domain->triclinic,sizeof(int),1,fp);
...
void DumpSsaTsdpd::header_item(bigint ndump)
{
...
void DumpSsaTsdpd::header_item_triclinic(bigint ndump)
{
  fprintf(fp,"ITEM: TIMESTEP\n");
  fprintf(fp,BIGINT_FORMAT "\n",update->ntimestep);
  fprintf(fp,"ITEM: NUMBER OF ATOMS\n");
...
void DumpSsaTsdpd::pack(tagint *ids)
{
...
int DumpSsaTsdpd::convert_string(int n, double *mybuf)
{
  int i,j;
  int offset = 0;
  int m = 0;
...
void DumpSsaTsdpd::write_data(int n, double *mybuf)
{
...
void DumpSsaTsdpd::write_binary(int n, double *mybuf)
{
  n *= size_one;
...
void DumpSsaTsdpd::write_string(int n, double *mybuf)
{
...
void DumpSsaTsdpd::write_lines(int n, double *mybuf)
{
  int i,j;
  int m = 0;
  for (i = 0; i < n; i++) {
...
int DumpSsaTsdpd::parse_fields(int narg, char **arg)
{
  // customize by adding to if statement
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_id;
      if (sizeof(tagint) == sizeof(smallint)) vtype[i] = INT;
      else vtype[i] = BIGINT;
    } else if (strcmp(arg[iarg],"mol") == 0) {
      if (!atom->molecule_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_molecule;
      if (sizeof(tagint) == sizeof(smallint)) vtype[i] = INT;
      else vtype[i] = BIGINT;
    } else if (strcmp(arg[iarg],"proc") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_proc;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"procp1") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_procp1;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"type") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_type;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"element") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_type;
      vtype[i] = STRING;
    } else if (strcmp(arg[iarg],"mass") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_mass;
      vtype[i] = DOUBLE;

    //added
    } else if (strncmp(arg[iarg],"C_",2) == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_C;
      vtype[i] = DOUBLE;
      ...
      if (ptr) {
        if (suffix[strlen(suffix)-1] != ']')
          error->all(FLERR,"Invalid attribute in dump custom command");
	if (atoi(ptr+1) > atom->num_tdpd_species - 1)
	  error->all(FLERR,"Argument in C_[] is greater than the number of TSDPD species");
    ...
    //added
    } else if (strncmp(arg[iarg],"Cd_",2) == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_C;

      if (atom->Cd_concentration_flag == 1) {
        vtype[i] = DOUBLE;
      }
      else{
        vtype[i] = INT;
      ...
      if (ptr) {
        if (suffix[strlen(suffix)-1] != ']')
          error->all(FLERR,"Invalid attribute in dump custom command");
	if (atoi(ptr+1) > atom->num_ssa_species - 1)
	  error->all(FLERR,"Argument in Cd_[] is greater than the number of SSA species");
        argindex[i] = atoi(ptr+1);
        *ptr = '\0';
      } else argindex[i] = 0;
      delete [] suffix;
      ...
      pack_choice[i] = &DumpSsaTsdpd::pack_x;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"y") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_y;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"z") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_z;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xs") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_xs_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_xs;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ys") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_ys_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_ys;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zs") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_zs_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_zs;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_xu_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_xu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"yu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_yu_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_yu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_zu_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_zu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xsu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_xsu_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_xsu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ysu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_ysu_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_ysu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zsu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpSsaTsdpd::pack_zsu_triclinic;
      else pack_choice[i] = &DumpSsaTsdpd::pack_zsu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ix") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_ix;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"iy") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_iy;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"iz") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_iz;
      vtype[i] = INT;

    } else if (strcmp(arg[iarg],"vx") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_vx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"vy") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_vy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"vz") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_vz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fx") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_fx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fy") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_fy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fz") == 0) {
      pack_choice[i] = &DumpSsaTsdpd::pack_fz;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"q") == 0) {
      if (!atom->q_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_q;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"mux") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_mux;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"muy") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_muy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"muz") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_muz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"mu") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_mu;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"radius") == 0) {
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_radius;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"diameter") == 0) {
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_diameter;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegax") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_omegax;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegay") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_omegay;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegaz") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_omegaz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomx") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_angmomx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomy") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_angmomy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomz") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_angmomz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqx") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_tqx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqy") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_tqy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqz") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpSsaTsdpd::pack_tqz;
      vtype[i] = DOUBLE;
    ...
    // if no trailing [], then arg is set to 0, else arg is int between []
      ...
      pack_choice[i] = &DumpSsaTsdpd::pack_compute;
      vtype[i] = DOUBLE;
    ...
    // if no trailing [], then arg is set to 0, else arg is between []
      ...
      pack_choice[i] = &DumpSsaTsdpd::pack_fix;
      vtype[i] = DOUBLE;
    ...
    // variable value = v_name
      ...
      pack_choice[i] = &DumpSsaTsdpd::pack_variable;
      vtype[i] = DOUBLE;
    ...
    // custom per-atom floating point value = d_ID
      ...
      pack_choice[i] = &DumpSsaTsdpd::pack_custom;
      vtype[i] = DOUBLE;
...
int DumpSsaTsdpd::add_compute(char *id)
{
  int icompute;
...
int DumpSsaTsdpd::add_fix(char *id)
{
  int ifix;
...
int DumpSsaTsdpd::add_variable(char *id)
{
  int ivariable;
...
int DumpSsaTsdpd::add_custom(char *id, int flag)
{
  int icustom;
...
int DumpSsaTsdpd::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    //added
    else if (strcmp(arg[1],"C") == 0) thresh_array[nthresh] = VX;
    else if (strcmp(arg[1],"Cd") == 0) thresh_array[nthresh] = ID;
...
bigint DumpSsaTsdpd::memory_usage()
{
...
void DumpSsaTsdpd::pack_compute(int n)
{
...
void DumpSsaTsdpd::pack_fix(int n)
{
...
void DumpSsaTsdpd::pack_variable(int n)
{
...
void DumpSsaTsdpd::pack_custom(int n)
{
...
void DumpSsaTsdpd::pack_id(int n)
{
  tagint *tag = atom->tag;
...
void DumpSsaTsdpd::pack_molecule(int n)
{
  tagint *molecule = atom->molecule;
...
void DumpSsaTsdpd::pack_proc(int n)
{
  for (int i = 0; i < nchoose; i++) {
    buf[n] = me;
    n += size_one;
...
void DumpSsaTsdpd::pack_procp1(int n)
{
  for (int i = 0; i < nchoose; i++) {
    buf[n] = me+1;
    n += size_one;
...
void DumpSsaTsdpd::pack_type(int n)
{
  int *type = atom->type;
...
void DumpSsaTsdpd::pack_mass(int n)
{
  int *type = atom->type;
  double *mass = atom->mass;
...
/*----------------------------------------------------------------------- */
//added
void DumpSsaTsdpd::pack_C(int n)
{
  double **C = atom->C;
  int index = argindex[n];
  for (int i = 0; i < nchoose; i++) {
          buf[n] = C[clist[i]][index];
          n += size_one;
 }
}
...
/*----------------------------------------------------------------------- */
//added
void DumpSsaTsdpd::pack_Cd(int n)
{
  int **Cd = atom->Cd;
  int index = argindex[n];
  double *mass = atom->mass;
  ...
  if (atom->Cd_concentration_flag == 1) {
    for (int i = 0; i < nchoose; i++) {
      buf[n] = (double) Cd[clist[i]][index] * rho[clist[i]] / mass[type[clist[i]]];
      n += size_one;
    }
  }
...
void DumpSsaTsdpd::pack_x(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_y(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_z(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_xs(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_ys(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_zs(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_xs_triclinic(int n)
{
  int j;
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_ys_triclinic(int n)
{
  int j;
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_zs_triclinic(int n)
{
  double **x = atom->x;
...
void DumpSsaTsdpd::pack_xu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_yu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_zu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_xu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_yu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_zu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_xsu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_ysu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_zsu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_xsu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_ysu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_zsu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_ix(int n)
{
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_iy(int n)
{
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_iz(int n)
{
  imageint *image = atom->image;
...
void DumpSsaTsdpd::pack_vx(int n)
{
  double **v = atom->v;
...
void DumpSsaTsdpd::pack_vy(int n)
{
  double **v = atom->v;
...
void DumpSsaTsdpd::pack_vz(int n)
{
  double **v = atom->v;
...
void DumpSsaTsdpd::pack_fx(int n)
{
  double **f = atom->f;
...
void DumpSsaTsdpd::pack_fy(int n)
{
  double **f = atom->f;
...
void DumpSsaTsdpd::pack_fz(int n)
{
  double **f = atom->f;
...
void DumpSsaTsdpd::pack_q(int n)
{
  double *q = atom->q;
...
void DumpSsaTsdpd::pack_mux(int n)
{
  double **mu = atom->mu;
...
void DumpSsaTsdpd::pack_muy(int n)
{
  double **mu = atom->mu;
...
void DumpSsaTsdpd::pack_muz(int n)
{
  double **mu = atom->mu;
...
void DumpSsaTsdpd::pack_mu(int n)
{
  double **mu = atom->mu;
...
void DumpSsaTsdpd::pack_radius(int n)
{
  double *radius = atom->radius;
...
void DumpSsaTsdpd::pack_diameter(int n)
{
  double *radius = atom->radius;
...
void DumpSsaTsdpd::pack_omegax(int n)
{
...
void DumpSsaTsdpd::pack_omegay(int n)
{
  double **omega = atom->omega;
...
void DumpSsaTsdpd::pack_omegaz(int n)
{
  double **omega = atom->omega;
...
void DumpSsaTsdpd::pack_angmomx(int n)
{
...
void DumpSsaTsdpd::pack_angmomy(int n)
{
  double **angmom = atom->angmom;
...
void DumpSsaTsdpd::pack_angmomz(int n)
{
  double **angmom = atom->angmom;
...
void DumpSsaTsdpd::pack_tqx(int n)
{
...
void DumpSsaTsdpd::pack_tqy(int n)
{
  double **torque = atom->torque;
...
void DumpSsaTsdpd::pack_tqz(int n)
{
  double **torque = atom->torque;
