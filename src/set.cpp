...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     MESO_E,MESO_CV,MESO_RHO,SMD_MASS_DENSITY,SMD_CONTACT_RADIUS,DPDTHETA,SSATDPDC,
     SSATSDPD_E,SSATSDPD_CV,SSATSDPD_RHO,SSATSDPD_C,SSATSDPD_CD,INAME,DNAME};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
	...
	//added (for ssa/tdpd model)
    } else if (strcmp(arg[iarg],"ssa_tdpd/C") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strcmp(arg[iarg+2],"NULL") == 0) dvalue = 0.0;
      else if (strstr(arg[iarg+2],"v_") == arg[iarg+2]) varparse(arg[iarg+2],2);
      else {
        ivalue = force->inumeric(FLERR,arg[iarg+1]);
        dvalue = force->numeric(FLERR,arg[iarg+2]);
      if (dvalue < 0.0) error->all(FLERR,"Illegal set command");
      if (ivalue < 0) error->all(FLERR,"Illegal set command");
      }
      if (!atom->tdpd_flag)
        error->all(FLERR,"Cannot set ssa_tdpd/C for this atom style");
      set(SSATDPDC);
      iarg += 3;


	//added (for ssa/tsdpd model)
    } else if (strcmp(arg[iarg],"ssa_tsdpd/e") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->e_flag)
        error->all(FLERR,"Cannot set ssa_tsdpd/e for this atom style");
      set(SSATSDPD_E);
      iarg += 2;

    } else if (strcmp(arg[iarg],"ssa_tsdpd/cv") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->cv_flag)
            error->all(FLERR,"Cannot set ssa_tsdpd/cv for this atom style");
      set(SSATSDPD_CV);
      iarg += 2;

    } else if (strcmp(arg[iarg],"ssa_tsdpd/rho") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->rho_flag)
        error->all(FLERR,"Cannot set ssa_tsdpd/rho for this atom style");
      set(SSATSDPD_RHO);
      iarg += 2;

    } else if (strcmp(arg[iarg],"ssa_tsdpd/C") == 0) {
      if (strcmp(arg[iarg+2],"NULL") == 0) dvalue = 0.0;
      else if (strstr(arg[iarg+2],"v_") == arg[iarg+2]) varparse(arg[iarg+2],2);
      else {
        ivalue = force->inumeric(FLERR,arg[iarg+1]);
      ...
      if (!atom->tsdpd_flag)
        error->all(FLERR,"Cannot set ssa_tsdpd/C for this atom style");
      set(SSATSDPD_C);
      iarg += 3;


      //added (for ssa/tsdpd stochastic chemical reactions)
    } else if (strcmp(arg[iarg],"ssa_tsdpd/Cd") == 0) {
      if (strcmp(arg[iarg+2],"NULL") == 0) ivalue = 0;
      else if (strstr(arg[iarg+2],"v_") == arg[iarg+2]) varparse(arg[iarg+2],2);
      else {
        ivalue = force->inumeric(FLERR,arg[iarg+1]);
        ivalue2 = force->numeric(FLERR,arg[iarg+2]);
      if (ivalue < 0) error->all(FLERR,"Illegal set command");
      if (ivalue2 < 0) error->all(FLERR,"Illegal set command");
      }
      if (!atom->tsdpd_flag)
        error->all(FLERR,"Cannot set ssa_tsdpd/Cd for this atom style");
      set(SSATSDPD_CD);
      iarg += 3;
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    else if (keyword == SSATSDPD_E) atom->e[i] = dvalue;
    else if (keyword == SSATSDPD_CV) atom->cv[i] = dvalue;
    else if (keyword == SSATSDPD_RHO) atom->rho[i] = dvalue;
    ...
    // set concentration for dpd particle
    else if (keyword == SSATDPDC) {
	if (dvalue >= 0.0) atom->C[i][ivalue] = dvalue;
	}

    // set concentration for TSDPD species (deterministic)
    else if (keyword == SSATSDPD_C) {
	if (dvalue >=0.0) atom->C[i][ivalue] = dvalue;
	 }

    // set concentration for SSA species (stochastic)
    else if (keyword == SSATSDPD_CD) {
	if (ivalue2 >=0) atom->Cd[i][ivalue] = ivalue2;
	 }
