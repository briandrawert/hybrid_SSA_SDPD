...
enum{V,F,V_K,C_K,DENSITY_NUMBER,DENSITY_MASS,MASS,TEMPERATURE,COMPUTE,FIX,VARIABLE};
enum{SCALAR,VECTOR};
enum{SAMPLE,ALL};
enum{NOSCALE,ATOM};
  ...
  values_total(NULL), values_list(NULL)
{
  ...
  while (iarg < nargnew) {
	...
	//added
    } else if (strcmp(arg[iarg],"vx_k") == 0) {
      which[nvalues] = V;
      argindex[nvalues++] = 0;
    } else if (strcmp(arg[iarg],"vy_k") == 0) {
      which[nvalues] = V;
      argindex[nvalues++] = 1;
    } else if (strcmp(arg[iarg],"vz_k") == 0) {
      which[nvalues] = V;
      argindex[nvalues++] = 2;
...
void FixAveChunk::end_of_step()
{
  ...
  //kip if not step which requires doing s  ,xeck yf tis   ,x yn invalid manner
  ...
  for (m = 0; m < nvalues; m++) {
    ...
    // Added (Kernel interpolation of velocity and concentration)

    } else if (which[m] == V_K || which[m] == C_K ) {
      double **attribute;
      double **x = atom->x;
      double **v = atom->v;
      int ncoord_c = cchunk->ncoord;
      double **coord_c = cchunk->coord;
      double xtmp,ytmp,ztmp,delx,dely,delz,r,wrx,wry,wrz,wr;
 
      if (which[m] == V_K) attribute = atom->v;
      else attribute = atom->C;
      ...
      for (i = 0; i < nlocal; i++)
        if (mask[i] & groupbit && ichunk[i] > 0) {
          ...
          ytmp = x[i][1];
          ztmp = x[i][2];
          delx = abs(xtmp - coord_c[index][0]);
          dely = abs(ytmp - coord_c[index][1]);
          delz = abs(ztmp - coord_c[index][2]);
          r = sqrt(delx*delx + dely*dely + delz*delz);

          if (delx<2.0) {
              if  (delx>1.0)  wrx = 0.5*(2.0-delx)*(2.0-delx)*(1-delx);
              else wrx = 1.0 - (5.0/2.0)*(delx*delx) + (3.0/2.0)*(delx*delx*delx);
          }

          if (dely<2.0) {
              if  (dely>1.0)  wry = 0.5*(2.0-dely)*(2.0-dely)*(1-dely);
              else wry = 1.0 - (5.0/2.0)*(dely*dely) + (3.0/2.0)*(dely*dely*dely);
          }
	  ...
	  //if (delz<2.0) {
         ...
         values_one[index][m] += wr*attribute[i][j];
